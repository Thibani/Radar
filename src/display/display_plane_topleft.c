    ...
    game_t *game)
{
    ...
    if (game->i == true) {
        update_plane(*current_left, game, &entity->plane_toplft, entity);
    }
    ...
    entity_t *entity, int k)
{
    if (k == 0 && (*current_left)->status == FLY && calcul_screen_loc
        ...
        entity->plane_botlft = move_pln(*current_left, &entity->plane_botlft);
    ...
    entity_t *entity)
{
    ...
    if ((*current_left)->status == FLY && calcul_screen_loc((*current_left)
        ->object.position, sfRectangleShape_getPosition(game->top_rht),
        sfRectangleShape_getSize(game->top_rht))) {
    ...
    game_t *game, entity_t *entity)
{
    ...
    while (*cur_left != NULL) {
        ...
        if ((*cur_left)->status == DEAD || (*cur_left)->status == LAND) {
            del_funct(prev_left, cur_left, &entity->plane_toplft);
            k = 1;
        }
        if (k == 0 && (*cur_left)->status == TODELETE) {
            del_funct_bis(prev_left, cur_left, &entity->plane_toplft);
            k = 1;
        }
