    ...
    game_t *game)
{
    ...
    if (game->i == true) {
        update_plane(*current_right, game, &entity->plane_botrht, entity);
    }
    ...
    entity_t *entity)
{
    ...
    if (k == 0 && (*current_right)->status == FLY && calcul_screen_loc
        ...
        entity->plane_toprht = move_pln(*current_right, &entity->plane_toprht);
        (*current_right)->status = TODELETE;
        k = 1;
    }
    ...
    game_t *game, entity_t *entity)
{
    ...
    while (*cur_right != NULL) {
        ...
        if ((*cur_right)->status == DEAD || (*cur_right)->status == LAND) {
            del_funct(prev_right, cur_right, &entity->plane_botrht);
            k = 1;
        }
        if (k == 0 && (*cur_right)->status == TODELETE) {
            del_funct_bis(prev_right, cur_right, &entity->plane_botrht);
            k = 1;
        }
